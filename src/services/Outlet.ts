...
class Outlet {
  async getOutletByIds(ids: string[]): Promise<any> {
    const gets = await db.query(
      "SELECT outlet_id FROM mstr_outlet WHERE outlet_id IN(?)",
      {
        raw: true,
  ...
  async getOutletActive(req: Request): Promise<any> {
    const { query: qoc, params: poc } = FilterParams.query(
      req,
      queryOutletCount
    );

    const q = `SELECT (${qoc}) AS total_outlet, COUNT(DISTINCT tr.no_id) AS aktif FROM trx_transaksi AS tr INNER JOIN mstr_outlet AS o ON tr.no_id = o.outlet_id INNER JOIN ms_pulau_alias AS reg ON o.region_id = reg.pulau_id_alias INNER JOIN ms_dist_pic AS dp ON o.distributor_id = dp.distributor_id WHERE o.outlet_id IS NOT NULL`;
  ...
  async resetRegistration(): Promise<any> {
    return await db.query(
      "UPDATE mstr_outlet SET valid = ?, register_at = ? WHERE outlet_id = ?",
      {
        raw: true,
  ...
  async outletIsRegist(outlet_id: string): Promise<any> {
    const get: any = await db.query(
      "SELECT valid FROM mstr_outlet WHERE outlet_id = ?",
      {
        raw: true,
        type: QueryTypes.SELECT,
        replacements: [outlet_id],
