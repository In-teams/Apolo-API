...
class Sales {
	getTarget(req: Request): { query: string; params: string[] } {
		let {
			outlet_id,
			area_id,
			wilayah_id,
	...
	getAktual(req: Request): { query: string; params: string[] } {
		let {
			outlet_id,
			area_id,
			wilayah_id,
	...
	async getSalesByDistributor(req: Request) {
		...
		let { query: qTarget, params: pTarget } = this.getTarget(req);
		qTarget += ' AND o.`distributor_id` = d.`distributor_id`';
		let { query: qAktual, params: pAktual } = this.getAktual(req);
		...
		return await db.query(query, {
			...
			replacements: [...pTarget, ...pAktual, ...pAktual, ...pTarget, ...params],
		});
	}
}
