...
let queryAktualAndPoint: string =
  "SELECT `o`.`outlet_id`, MONTH(tr.tgl_transaksi) AS bulan, SUM(`sales`) AS `aktual`, SUM(`point_satuan`) AS `poin` FROM `trx_transaksi` AS `tr`INNER JOIN `trx_transaksi_barang` AS `trb` ON `tr`.`kd_transaksi` = `trb`.`kd_transaksi`INNER JOIN `mstr_outlet` AS `o` ON `tr`.`no_id` = `o`.`outlet_id`INNER JOIN `ms_dist_pic` AS `pic` ON `o`.`distributor_id` = `pic`.`distributor_id`INNER JOIN `ms_pulau_alias` AS `r` ON `o`.`region_id` = `r`.`pulau_id_alias`INNER JOIN `ms_head_region` AS `hr` ON `r`.`head_region_id` = `hr`.`head_region_id` WHERE o.outlet_id IS NOT NULL";
