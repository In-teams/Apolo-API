...
const getSalesByHirarki = (
  ...
  const { wilayah_id, ass_id, asm_id } = payload;
  ...
  if (hirarki === "wilayah") {
    ...
    if (isCount) {
      groupBy =
        "SELECT COUNT(DISTINCT mhr.head_region_id) AS total FROM mstr_outlet AS o " +
      ...
      groupBy =
        "mhr.head_region_id, mhr.head_region_name, mhr.head_region_name";
    }
    ...
    if (isCount) {
      groupBy =
        "SELECT COUNT(DISTINCT o.distributor_id) AS total FROM mstr_outlet AS o " +
  ...
  const aktual = getAktual(payload);
  const redeem = getRedeem(payload);
  const regist = getRegist(payload);
  const totalTarget = getTarget(payload, "SUM(st.target_sales) AS target");
  const outlets = filterParams.sales(getOutlet, payload);
  let query = "";
  if (!isCount) {
    query = `SELECT ${cols}, 
...
class Sales {
  async getOutletCount(req: Request): Promise<{ target: number }[]> {
    const { query: newQuery, params } = filterParams.count(
      req,
      queryOutletCount
    );
  ...
  async getTarget(req: Request): Promise<{ target: number }[]> {
    const query =
      "SELECT SUM(st.target_sales) AS target FROM ( SELECT * FROM mstr_sales_target UNION SELECT * FROM mstr_sales_target2 UNION SELECT * FROM mstr_sales_target3 UNION SELECT * FROM mstr_sales_target4 ) AS st INNER JOIN mstr_outlet AS ou ON ou.outlet_id = st.outlet_id INNER JOIN ms_pulau_alias AS r ON ou.region_id = r.pulau_id_alias INNER JOIN ms_dist_pic AS pic ON ou.distributor_id = pic.distributor_id INNER JOIN ms_bulan AS b ON b.bulan = st.month_target WHERE st.outlet_id IS NOT NULL";
    const { query: q, params: p } = filterParams.target(req, query);
  ...
  async getSalesByArea(req: Request): Promise<salesByHirarki[]> {
    const { sort } = req.validated;
    const qTarget = queryTarget + " AND ou.city_id_alias = ci.city_id_alias";
    const qAktual = queryAktual + " AND ou.city_id_alias = ci.city_id_alias";

    const { query: qt, params: pt } = filterParams.target(req, qTarget);
    const { query: qa, params: pa } = filterParams.aktual(req, qAktual);

    const query = `SELECT ci.city_name_alias as city, (${qt}) AS target, (${qa}) AS aktual, (ROUND(((${qAktual})/(${qt})* 100), 2 )) AS pencapaian, COUNT(o.outlet_id) AS outlet FROM mstr_outlet AS o INNER JOIN ms_city_alias as ci ON ci.city_id_alias = o.city_id_alias INNER JOIN ms_pulau_alias AS reg ON o.region_id = reg.pulau_id_alias INNER JOIN ms_dist_pic AS dp ON o.distributor_id = dp.distributor_id WHERE ci.city_id_alias IS NOT NULL`;
  ...
  async getSalesByOutlet(req: Request): Promise<salesByHirarki[]> {
    ...
    const sortBy = sort.split(",")[0];
    const sortDesc = sort.split(",")[1];
    const qTarget = queryTarget + " AND ou.outlet_id = o.outlet_id";
    const qAktual = queryAktual + " AND ou.outlet_id = o.outlet_id";
    ...
    const query = `SELECT o.outlet_name as outlet_name, (${qt}) AS target, (${qa}) AS aktual, (ROUND(((${qa})/(${qt})* 100), 2 )) AS pencapaian, COUNT(o.outlet_id) AS outlet FROM mstr_outlet AS o INNER JOIN ms_pulau_alias AS reg ON o.region_id = reg.pulau_id_alias INNER JOIN ms_dist_pic AS dp ON o.distributor_id = dp.distributor_id WHERE o.outlet_id IS NOT NULL`;
  ...
  async getSalesByRegion(req: Request): Promise<salesByHirarki[]> {
    const { sort } = req.validated;
    const qTarget = queryTarget + " AND ou.region_id = reg.pulau_id_alias";
    const qAktual = queryAktual + " AND ou.region_id = reg.pulau_id_alias";

    const { query: qt, params: pt } = filterParams.target(req, qTarget);
    const { query: qa, params: pa } = filterParams.aktual(req, qAktual);

    const query = `SELECT reg.nama_pulau_alias AS region, (${qt}) AS target, (${qa}) AS aktual, (ROUND(((${qa})/(${qt})* 100), 2 )) AS pencapaian, COUNT(o.outlet_id) AS outlet FROM mstr_outlet AS o INNER JOIN ms_pulau_alias AS reg ON o.region_id = reg.pulau_id_alias INNER JOIN ms_dist_pic AS dp ON o.distributor_id = dp.distributor_id WHERE reg.pulau_id_alias IS NOT NULL`;
  ...
  async getSalesByHR(req: Request): Promise<salesByHirarki[]> {
    const { sort } = req.validated;
    const qTarget = queryTarget + " AND r.head_region_id = mhr.head_region_id";
    const qAktual = queryAktual + " AND r.head_region_id = mhr.head_region_id";

    const { query: qt, params: pt } = filterParams.target(req, qTarget);
    const { query: qa, params: pa } = filterParams.aktual(req, qAktual);

    const query = `SELECT mhr.head_region_name AS wilayah, mhr.head_region_id AS wilayah_id, (${qt}) AS target, (${qa}) AS aktual, (ROUND(((${qa})/(${qt})* 100), 2 )) AS pencapaian, COUNT(o.outlet_id) AS outlet FROM mstr_outlet AS o INNER JOIN ms_pulau_alias AS reg ON o.region_id = reg.pulau_id_alias INNER JOIN ms_head_region AS mhr ON mhr.head_region_id = reg.head_region_id INNER JOIN ms_dist_pic AS dp ON o.distributor_id = dp.distributor_id WHERE mhr.head_region_id IS NOT NULL`;
  ...
  async getSalesByASS(req: Request): Promise<salesByHirarki[]> {
    const { sort } = req.validated;
    const qTarget = queryTarget + " AND pic.ass_id = mp.kode_pic";
    const qAktual = queryAktual + " AND pic.ass_id = mp.kode_pic";
  ...
  async getSalesByAchiev(data: any) {
    const sub = getSalesByHirarki("outlet", data, false, false);
    const totalTarget = getTarget(data, "SUM(st.target_sales) AS target");
  ...
  async getSummaryPerQuarter(req: Request) {
    ...
    const query = `SELECT b.bulan, aktual, poin, SUM(target) AS target, IFNULL(outlets, 0) AS outlets FROM mstr_outlet AS o LEFT JOIN(${qt}) AS mst ON mst.outlet_id = o.outlet_id LEFT JOIN(${qap}) AS trb ON trb.bulan = mst.bulan INNER JOIN ms_bulan AS b ON b.id = mst.bulan INNER JOIN ms_pulau_alias AS reg ON o.region_id = reg.pulau_id_alias INNER JOIN ms_dist_pic AS dp ON o.distributor_id = dp.distributor_id WHERE o.outlet_id IS NOT NULL`;
  ...
  async getSummaryPerYears(req: Request) {
    ...
    const query = `SELECT SUM(aktual) AS aktual, SUM(poin) AS poin, SUM(target) AS target FROM mstr_outlet AS o LEFT JOIN(${qt}) AS mst ON mst.outlet_id = o.outlet_id LEFT JOIN(${qap}) AS trb ON trb.bulan = mst.bulan INNER JOIN ms_dist_pic AS dp ON o.distributor_id = dp.distributor_id INNER JOIN ms_pulau_alias AS reg ON o.region_id = reg.pulau_id_alias INNER JOIN ms_bulan AS b ON b.id = mst.bulan WHERE o.outlet_id IS NOT NULL`;
  ...
  async getSummary(req: Request) {
    const { query: qt, params: pt } = filterParams.target(req, queryTarget);
    const { query: qa, params: pa } = filterParams.aktual(req, queryAktual);
    const { query: qo, params: po } = filterParams.count(req, queryOutletCount);

    const query = `SELECT DISTINCT (${qa}) AS aktual, (${qt}) AS target, (${qo}) AS total_outlet FROM mstr_outlet AS o INNER JOIN ms_pulau_alias AS reg ON o.region_id = reg.pulau_id_alias INNER JOIN ms_dist_pic AS dp ON o.distributor_id = dp.distributor_id WHERE o.outlet_id IS NOT NULL`;
  ...
  async getTargetByHirarki(req: Request, type?: string) {
    ...
    const q = `SELECT SUM(mst.target_sales) AS target, ${select}, COUNT(DISTINCT mst.outlet_id) AS outlet FROM ( SELECT * FROM mstr_sales_target UNION SELECT * FROM mstr_sales_target2 UNION SELECT * FROM mstr_sales_target3 UNION SELECT * FROM mstr_sales_target4 ) AS mst INNER JOIN ms_bulan AS b ON b.bulan = mst.month_target INNER JOIN mstr_outlet as ou ON ou.outlet_id = mst.outlet_id INNER JOIN mstr_distributor AS d ON d.distributor_id = ou.distributor_id INNER JOIN ms_city_alias AS c ON c.city_id_alias = ou.city_id_alias INNER JOIN ms_dist_pic AS pic ON ou.distributor_id = pic.distributor_id INNER JOIN ms_pic AS p ON p.kode_pic = pic.asm_id INNER JOIN ms_pic AS ass ON ass.kode_pic = pic.ass_id INNER JOIN ms_pulau_alias AS r ON ou. region_id = r.pulau_id_alias INNER JOIN ms_head_region AS hr ON hr.head_region_id = r.head_region_id WHERE mst.outlet_id IS NOT NULL`;

    const { query, params } = filterParams.target(req, q);
  ...
  async getAktualByHirarki(
    ...
    const q = `SELECT SUM(trb.sales) AS aktual, ${select}, COUNT(DISTINCT tr.no_id) AS outlet FROM trx_transaksi AS tr INNER JOIN trx_transaksi_barang AS trb ON tr.kd_transaksi = trb.kd_transaksi INNER JOIN mstr_outlet AS ou ON ou.outlet_id = tr.no_id INNER JOIN mstr_distributor AS d ON d.distributor_id = ou.distributor_id INNER JOIN ms_city_alias AS c ON c.city_id_alias = ou.city_id_alias INNER JOIN ms_dist_pic AS pic ON ou.distributor_id = pic.distributor_id INNER JOIN ms_pic AS p ON p.kode_pic = pic.asm_id INNER JOIN ms_pic AS ass ON ass.kode_pic = pic.ass_id INNER JOIN ms_pulau_alias AS r ON ou.region_id = r.pulau_id_alias INNER JOIN ms_head_region AS hr ON hr.head_region_id = r.head_region_id WHERE tr.no_id IS NOT NULL`;

    const { query, params } = filterParams.aktual(req, q);
  ...
  async getAktualByMonth(req: Request): Promise<any> {
    const q = `SELECT SUM(trb.sales) AS aktual, COUNT(DISTINCT tr.no_id) AS outlet, MONTH(tr.tgl_transaksi) AS bulan FROM trx_transaksi AS tr INNER JOIN trx_transaksi_barang AS trb ON tr.kd_transaksi = trb.kd_transaksi INNER JOIN mstr_outlet AS ou ON ou.outlet_id = tr.no_id INNER JOIN mstr_distributor AS d ON d.distributor_id = ou.distributor_id INNER JOIN ms_city_alias AS c ON c.city_id_alias = ou.city_id_alias INNER JOIN ms_dist_pic AS pic ON ou.distributor_id = pic.distributor_id INNER JOIN ms_pulau_alias AS r ON ou.region_id = r.pulau_id_alias INNER JOIN ms_head_region AS hr ON hr.head_region_id = r.head_region_id WHERE tr.no_id IS NOT NULL`;
  ...
  async getTargetByMonth(req: Request) {
    const q = `SELECT SUM(mst.target_sales) AS target, COUNT(DISTINCT mst.outlet_id) AS outlet, mst.month_target AS bulan FROM ( SELECT * FROM mstr_sales_target UNION SELECT * FROM mstr_sales_target2 UNION SELECT * FROM mstr_sales_target3 UNION SELECT * FROM mstr_sales_target4 ) AS mst INNER JOIN ms_bulan AS b ON b.bulan = mst.month_target INNER JOIN mstr_outlet as ou ON ou.outlet_id = mst.outlet_id INNER JOIN mstr_distributor AS d ON d.distributor_id = ou.distributor_id INNER JOIN ms_city_alias AS c ON c.city_id_alias = ou.city_id_alias INNER JOIN ms_dist_pic AS pic ON ou.distributor_id = pic.distributor_id INNER JOIN ms_pulau_alias AS r ON ou. region_id = r.pulau_id_alias INNER JOIN ms_head_region AS hr ON hr.head_region_id = r.head_region_id WHERE mst.outlet_id IS NOT NULL`;
