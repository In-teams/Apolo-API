...
class SubDistrict {
  async getNameById(id: string): Promise<any> {
    const query = "SELECT name FROM subdistricts WHERE id = ?";
  ...
  async get(data: any): Promise<any> {
    const query =
      "SELECT DISTINCT sd.* FROM provinces AS p INNER JOIN cities AS c ON p.`id` = c.`provinces_id` INNER JOIN districts AS d ON d.`cities_id` = c.`id` INNER JOIN subdistricts AS sd ON sd.`districts_id` = d.`id`";
