...
class Distributor {
  async get(req: Request): Promise<any> {
    ...
    let query =
        'select distinct d.distributor_name, d.distributor_id from mstr_distributor as d INNER JOIN mstr_outlet as o on d.distributor_id = o.distributor_id INNER JOIN ms_pulau_alias as r on o.region_id = r.pulau_id_alias INNER JOIN ms_user_scope as us on o.outlet_id = us.scope INNER JOIN ms_dist_pic as dp on o.distributor_id = dp.distributor_id WHERE d.distributor_id IS NOT NULL';
    ...
    return await db.query(query + ' order by d.distributor_id ASC', {
        raw: true,
