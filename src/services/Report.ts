...
const getRedeemReportQuery = async (
  ...
  if (status_terima) {
    if (status_terima === 1) {
      query +=
        " AND l.`tgl_confirm` IS NULL AND j.`tanggal` IS NULL AND h.tanggal_kirim IS NULL AND h.tanggal_terima IS NULL";
    } else if (status_terima === 2) {
      query +=
        " AND l.`tgl_confirm` IS NOT NULL AND j.`tanggal` IS NULL AND h.tanggal_kirim IS NULL AND h.tanggal_terima IS NULL";
    } else if (status_terima === 3) {
      query +=
        " AND l.`tgl_confirm` IS NOT NULL AND j.`tanggal` IS NOT NULL AND h.tanggal_kirim IS NULL AND h.tanggal_terima IS NULL";
    } else if (status_terima === 4) {
      query +=
        " AND l.`tgl_confirm` IS NOT NULL AND j.`tanggal` IS NOT NULL AND h.tanggal_kirim IS NOT NULL AND h.tanggal_terima IS NOT NULL";
    }
  }
