...
const getPointActivityQuery = async (
  ...
  const col = isCount
    ...
    : "a.outlet_id, o.outlet_name, a.target, IFNULL(b.aktual, 0) AS aktual, CONCAT(IFNULL(TRUNCATE(((IFNULL(aktual, 0) / target) * 100), 2), 0), '%') AS pencapaian, IFNULL(poin_achieve, 0) AS poin_achieve, IFNULL(poin_redeem, 0) AS poin_redeem, IFNULL(poin_bonus, 0) AS poin_bonus, IFNULL(poin_reguler, 0) AS poin_reguler, (IFNULL(poin_achieve, 0) - IFNULL(poin_redeem, 0)) AS sisa, IFNULL(f.level, 'Level 1') AS level, IFNULL(f.status, 'Level 1A - Formulir Tidak Ada') AS status";
