...
class Region {
	async get(req: Request): Promise<any> {
		...
		let query =
			'select distinct r.nama_pulau_alias as region_name, r.pulau_id_alias as region_id FROM ms_pulau_alias as r INNER JOIN mstr_outlet as o on r.pulau_id_alias = o.region_id INNER JOIN ms_user_scope as us on o.outlet_id = us.scope INNER JOIN ms_dist_pic as dp on o.distributor_id = dp.distributor_id WHERE r.pulau_id_alias IS NOT NULL';
		...
		return await db.query(query + ' order by r.pulau_id_alias ASC', {
			raw: true,
